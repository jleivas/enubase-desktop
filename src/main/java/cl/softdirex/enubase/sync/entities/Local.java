...
public class Local implements InterfaceSync {
    ...
    public boolean add(Object objectParam) {
        ...
        try{
            ...
            if(objectParam instanceof Armazon){
                ...
                Notification.showMsg("Error inseperado en la operación", "armazon: " + object.getCod()+ "\nNo se pudo insertar.", 3);
                return false;
            }
            ...
            if(objectParam instanceof Cristal){
                ...
                Notification.showMsg("Error inseperado en la operación", "Cristal: " + object.getNombre() + "\nId: " + object.getId() + "\nNo se pudo insertar.", 3);
                return false;
            }
            if(objectParam instanceof CuotasConvenio){
                ...
                Notification.showMsg("Error inseperado en la operación", "CuotasConvenio: " + object.getCod()+ "\nNo se pudo insertar.", 3);
                return false;
            }
            ...
            if(objectParam instanceof Doctor){
                ...
                Notification.showMsg("Error inseperado en la operación", "Profesional: " + object.getNombre()+ "\nId: " + object.getCod()+ "\nNo se pudo insertar.", 3);
                return false;
            }
            ...
            if(objectParam instanceof Institucion){
                ...
                Notification.showMsg("Error inseperado en la operación", "Institucion: " + object.getNombre()+ "\nId: " + object.getCod()+ "\nNo se pudo insertar.", 3);
                return false;
            }
            ...
            if(objectParam instanceof Lente){
                ...
                Notification.showMsg("Error inseperado en la operación", "Lente: " + object.getCod()+ "\nId: " + object.getCod() + "\nNo se pudo insertar.", 3);
                return false;
            }
     ...
     * id_user: GV.convertVentaIdParamToUser(idParam),
     * rut_cliente: GV.convertVentaIdParamToClient(idParam)
    ...
    public ArrayList<Object> listar(String idParam, Object type) {
        ...
        try {
            if(type instanceof Venta){
                if(!GV.ventaIdParamIsVentaList(idParam) && !GV.ventaIdParamIsIdVenta(idParam)){
                    return listar(idParam,new ResF());
                }
                String sql = getSqlVenta()+" WHERE venta.ven_id='" + idParam + "'";
                sql = (idParam.equals("-2"))?getSqlVenta():sql;
                if(GV.ventaIdParamIsVentaList(idParam)){
                    sql=getSqlVenta()+GV.cleanIdParam(idParam);
                }
                    ...
                    while (datos.next()) {
                        ...
                        String IdVentaLj;
                        try{IdVentaLj = datos.getString("ven_id");}catch (Exception e){IdVentaLj="";}
                        int estaLj;
                        ...
                        Armazon lejos = new Armazon(codLj, tipoLj, marcaLj, 
                                ...
                                IdVentaLj, estaLj, lastUpdLj, lastHouLj);
                        /*--------------------------------------------------*/
                        /*-----------------ARMAZON CERCA--------------------*/
                        String codCr;
                        ...
                        String IdVentaCr;
                        try{IdVentaCr = datos.getString("ven_id");}catch (Exception e){IdVentaCr="";}
                        int estaCr;
                        ...
                        Armazon cerca = new Armazon(codCr, tipoCr, marcaCr, 
                                ...
                                IdVentaCr, estaCr, lastUpdCr, lastHouCr);
                        /*--------------------------------------------------*/
                        /*----------------------USER------------------------*/
                        int idUs;
            ...
            if(type instanceof ResF){
                ...
                if(!GV.ventaIdParamIsIdVenta(idParam)){
                    sql = (GV.ventaIdParamIsClient(idParam)) ? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    ...
                    + "FROM venta WHERE cliente_cli_rut='" + GV.cleanIdParam(idParam) + "'":sql;
                    sql = (GV.ventaIdParamIsUser(idParam)) ? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE usuario_us_id=" + GV.cleanIdParam(idParam) + "":sql;
                    sql = (GV.ventaIdParamIsTableList(idParam)) ? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE ven_estado=" + GV.cleanIdParam(idParam) + "":sql;
                    sql = (GV.ventaIdParamIsTableList(idParam) && GV.strToNumber(idParam)<0)? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE ven_estado <= 0":sql;
                    sql = (GV.ventaIdParamIsTableList(idParam) && GV.strToNumber(idParam)==0)? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE ven_estado > 0":sql;
                    sql = (GV.ventaIdParamIsDateList(idParam))? "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE ven_fecha BETWEEN '"+GV.cleanIdParam(idParam.substring(0, idParam.lastIndexOf("/")))
                    +"' and '"+GV.cleanIdParam(idParam.substring(idParam.lastIndexOf("/")+1))+"' ORDER BY ven_fecha DESC":sql;
                    //return lista de ResF
                }else{
                    return listar(idParam, new Venta());//retornara una lista de Ventas
                }
                if (idParam.equals("0")) {
                    sql = "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
                    + "(SELECT cli_comuna FROM cliente WHERE cli_rut = cliente_cli_rut) as comuna,"
                    + "(SELECT cli_ciudad FROM cliente WHERE cli_rut = cliente_cli_rut) as ciudad,"
                    + "(SELECT us_nombre FROM usuario WHERE us_id = usuario_us_id) as vendedor "
                    + "FROM venta WHERE ven_estado = 1";
                }
                if (idParam.equals("-1")) {
                    sql = "SELECT ven_id,ven_fecha,ven_valor_total,ven_descuento, ven_saldo,ven_estado,"
                    + "(SELECT cli_nombre FROM cliente WHERE cli_rut = cliente_cli_rut) as cliente,"
            ...
            if(type instanceof Armazon){
                ...
                String sql = "SELECT * FROM armazon WHERE venta_ven_id='" + id + "' AND arm_tipo="+subId;
                if (idParam.equals("0")) {
            ...
            if(type instanceof Cliente){
                ...
                + "(SELECT cliente.cli_last_update from cliente where cliente.cli_rut=venta.cliente_cli_rut) as cli_last_update, "
                + "(SELECT cliente.cli_last_hour from cliente where cliente.cli_rut=venta.cliente_cli_rut) as cli_last_hour "
                + "from venta where ven_estado="+GV.estadoVentaPaid()+" AND (ven_fecha_entrega < '"+GV.dateToString(new Date(), "yyyy-mm-dd")+"' OR ven_fecha_entrega = '"+GV.dateToString(new Date(), "yyyy-mm-dd")+"')";
                }
                PreparedStatement consulta = LcBd.obtener().prepareStatement(sql);
                ResultSet datos = consulta.executeQuery();
                while (datos.next()) {
                    if(!GC.getStr(datos.getString("cli_rut")).isEmpty()){
    ...
    /**
     * @param cod 
     * Armazon cod=idVenta, id=tipo de armazon
     * Cliente=>rut,
     * Cristal=>nombre,
     * Descuento=>nombre,
     * Doctor=>rut,
        ...
        try{
            if(type instanceof Venta){
                for (Object object : listar(cod, type)) {//id debe ser el rut del cliente
                    if (((Venta) object).getCod().equals(cod)) {
                        return object;
                    }
                }
                return null;
            ...
            if(type instanceof Armazon){
                String idVenta = GV.getStr(cod);
                int tipo = id;
                Armazon armazon = null;
                int auditoria =0;
                for (Object object : listar(GV.getStr(idVenta+"<"+tipo+">"), type)) {//id debe ser el rut del cliente
                    if(((Armazon) object).getIdVenta().equals(idVenta) && ((Armazon)object).getTipo() == tipo){
                        armazon= (Armazon)object;
                        auditoria++;
                    }
                }
                if(auditoria > 1){
                    Notification.showMsg("Es necesario corregir algunos datos", "La base de datos tiene conflicto con algunos items.\n"
                            + "Debe informar de este error a su proveedor de software con el siguiente id de seguimiento\n"
                            + "Identificador: ARM_FCH_ID_"+armazon.getIdVenta()+"\nDetalle: El sistema solo debe admitir dos armazones por venta.", 3);
                }
                return armazon;
            }
            if(type instanceof Cliente){
            ...
            if(type instanceof Despacho){
                ...
                for (Object object : listar(cod, type)) {//id debe ser el nombre del descuento
                    if (GV.strCompare(((Despacho) object).getIdVenta(),cod)) {
                ...
                if(auditoria > 1){
                    Notification.showMsg("Es necesario corregir algunos datos", "La base de datos tiene conflicto con algunos items.\n"
                            + "Debe informar de este error a su proveedor de software con el siguiente id de seguimiento\n"
                            + "Identificador: DSP_FCH_ID_"+despacho.getIdVenta()+"\nDetalle: El sistema solo debe admitir un despacho por venta.", 3);
                }
                return despacho;
            }
            if(type instanceof Doctor){
    ...
    public boolean exist(Object object) {
        ...
        if (object instanceof Venta) {
            ...
            if (getElement(((Venta) object).getCod(),0,object) != null) {
                return true;
            }
        }
        if (object instanceof Armazon) {
            
            if (getElement(((Armazon) object).getIdVenta(),((Armazon)object).getTipo(),object) != null) {
                return true;
            }
        }
        ...
        if (object instanceof Despacho) {
            ...
            if (getElement(((Despacho) object).getIdVenta(),0,object) != null) {
                return true;
            }
        }
    ...
    private String sqlUpdate(Object objectParam) {
        ...
        if(objectParam instanceof VentaDTO){
            ...
            return "UPDATE venta set ven_id = '" + object.getCod()
                        + "', ven_fecha = '" + sqlfecha1
    ...
    private String getSqlVenta() {
        return "SELECT " +
                ...
                "ven_obs, ven_valor_total, ven_descuento, ven_saldo, cliente_cli_rut, doctor_doc_rut, despacho_dsp_id, usuario_us_id, convenio_cnv_id, ven_estado," +
                ...
                " LEFT JOIN doctor ON doctor.doc_rut = venta.doctor_doc_rut" +
                " LEFT JOIN institucion ON institucion.ins_id = venta.institucion_ins_id" +
                " LEFT JOIN despacho ON despacho.dsp_id = venta.despacho_dsp_id" +
                " LEFT JOIN armazon as cerca ON cerca.venta_ven_id = venta.ven_id AND cerca.arm_tipo = 0" +
                " LEFT JOIN armazon as lejos ON lejos.venta_ven_id = venta.ven_id AND lejos.arm_tipo = 1" +
                " LEFT JOIN usuario ON usuario.us_id = venta.usuario_us_id" +
                " LEFT JOIN convenio ON convenio.cnv_id = venta.convenio_cnv_id ";
    }
    ...
    public String getSqlSync(Date paramDate, Object type) {
        ...
        try {
            if(type instanceof Venta || type instanceof VentaDTO){
                ...
                ResultSet datos = consulta.executeQuery();
                while (datos.next()) {
                    java.sql.Date sqlfecha = new java.sql.Date(datos.getDate("ven_last_update").getTime());
                    java.sql.Date sqlfecha2 = new java.sql.Date(datos.getDate("ven_fecha").getTime());
                    java.sql.Date sqlfecha3 = new java.sql.Date(datos.getDate("ven_fecha_entrega").getTime());
                    lista = lista + "UPDATE venta set ven_fecha = '" + sqlfecha2
                        + "', ven_fecha_entrega = '" + sqlfecha3
                        + "', ven_lugar_entrega = '" + datos.getString("ven_lugar_entrega")
                        + "', ven_hora_entrega = '" + datos.getString("ven_hora_entrega")
                        + "', ven_obs = '" + datos.getString("ven_obs")
                        + "', ven_valor_total = " + datos.getInt("ven_valor_total")
                        + ", ven_descuento = " + datos.getInt("ven_descuento")
                        + ", ven_saldo = " + datos.getInt("ven_saldo")
                        ...
                        + ", ven_estado = " + datos.getInt("ven_estado")
                        + ", ven_last_update = '" + sqlfecha
                        + "', ven_last_hour = " + datos.getInt("ven_last_hour")
                        + " WHERE ven_id = '" + datos.getString("ven_id")
                        + "' AND ((ven_last_update < '"+sqlfecha+"')OR"
                        + "(ven_last_update = '"+sqlfecha+"' AND ven_last_hour < "+datos.getInt("ven_last_hour")+"));";
                }
                LcBd.cerrar();
                return lista;
            }
            if(type instanceof Armazon){
                ...
                while (datos.next()) {
                    ...
                    lista = lista + "UPDATE armazon set arm_tipo = " + datos.getInt("arm_tipo")
                        ...
                        + ", venta_ven_id = '" + datos.getString("venta_ven_id")
