...
public class VProveedores extends javax.swing.JPanel {
    ...
    Dao load= new Dao();
    private static Proveedor stProveedor = null;
    TableRowSorter trs;
     ...
     */
    public VProveedores() {
        ContentAdmin.lblTitle.setText("Registro de proveedor.");
//        load.sincronize(new Proveedor());
        initComponents();
        ...
        ContentAdmin.lblTitle.setText("Proveedores");
        load();
        loadPanels(1);
    ...
    private void btnEliminarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnEliminarMouseClicked
        if(GV.tipoUserAdmin()){
            try{
                ...
                int fila = tblListar.getSelectedRow();
                String id = (tblListar.getValueAt(fila, 0).toString());
                if(OptionPane.getConfirmation("Eliminar Proveedor", "¿Esta seguro que desea eliminar el registro?", 2)){
                    cWT();
                    Proveedor temp = (Proveedor)load.get(id,0,new Proveedor());
                ...
                OptionPane.showMsg("Seleccione Proveedor","Error al cargar valores,\n"
    ...
    private void btnRestaurarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnRestaurarMouseClicked
        if(GV.tipoUserAdmin()){
            try{
                cWT();
                int fila = tblListar.getSelectedRow();
                String rut = (String)tblListar.getValueAt(fila, 0);
                if(OptionPane.getConfirmation("Confirmación de Proveedor", "¿Esta seguro que desea restaurar este registro?", 1)){
                    cWT();
                    Proveedor temp = (Proveedor)load.get(rut,0,new Proveedor());
                    if(!load.restoreOrDeleteFromUI(temp)){
                        cDF();
                        return;
                    }
    ...
    private void btnGuardarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnGuardarMouseClicked
        ...
        Proveedor proveedor= new Proveedor(rut, nombre, telefono, mail,web, direccion, comuna, ciudad, 1, null, 0);
        if(!load.addFromUI(proveedor)){
            cDF();
            return;
        }
        cargarDatos("0");
    ...
    private void btnModificarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnModificarMouseClicked
        ...
        stProveedor.setCiudad(txtCiudadU.getText());
        stProveedor.setComuna(txtComunaU.getText());
        stProveedor.setDireccion(txtDireccionU.getText());
        stProveedor.setEmail(email);
        stProveedor.setNombre(nombre);
        stProveedor.setTelefono(telefono);
        stProveedor.setWeb(txtWebU.getText());
        cWT();
        if(!load.updateFromUI(stProveedor)){
            cDF();
            return;
        }
        cargarDatos("0");
...
//                trs.setRowFilter(RowFilter.regexFilter(txtBuscar.getText(), 1));
    ...
    private void txtWebNKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtWebNKeyTyped
        int largo = 45;
        if(txtWebN.getText().length() >= largo){
            evt.consume();
            OptionPane.showMsg("Error de ingreso de datos", "El sitio web solo debe contener hasta 45 caracteres", 2);
        }
    ...
    private void txtWebUKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtWebUKeyTyped
        int largo = 45;
        if(txtWebU.getText().length() >= largo){
            evt.consume();
            OptionPane.showMsg("Error de ingreso de datos", "El sitio web solo debe contener hasta 45 caracteres", 2);
        }
    ...
    private void cargarDatos(String listar) {
        ...
        try{
            modelo.setNumRows(0);
            for (Object object : load.listar(listar, new Proveedor())) {
                Proveedor temp = (Proveedor)object;
            ...
            if(tblListar.getRowCount() == 0){
                GV.showMsgOnEmptyTable(cboMostrar, txtBuscar, "Proveedores");
            }
    ...
    private void abrirProveedor(String id) throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        loadPanels(2);
        stProveedor = (Proveedor)load.get(id,0,new Proveedor());
            if(stProveedor!=null){
                txtNombreU.setText(stProveedor.getNombre());
                txtCiudadU.setText(stProveedor.getCiudad());
                txtComunaU.setText(stProveedor.getComuna());
                txtDireccionU.setText(stProveedor.getDireccion());
                txtTelefonoU.setText(stProveedor.getTelefono());
                txtEmailU.setText(stProveedor.getEmail());
                txtWebU.setText(stProveedor.getWeb());
            }else{
                OptionPane.showMsg("Seleccione Proveedor","Error al cargar valores,\n"
                    + "es posible que no se haya seleccionado un registro\n"
                    + "o el valor seleccionado no tiene un identificador válido.",2);
