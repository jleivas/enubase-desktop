...
//                trs.setRowFilter(RowFilter.regexFilter(txtBuscar.getText(), 1));
    ...
    private void cargarCbos(){
        cboTipo2 = Icons.getCboTipoUsers(cboTipo2);
        cboTipo1 = Icons.getCboTipoUsers(cboTipo1);
    }
    ...
    private void cargarDatos(String listar) {
        ...
        try{
            ...
            if(tblListar.getRowCount() == 0){
                GV.showMsgOnEmptyTable(cboMostrar, txtBuscar, "Usuarios");
            }
    ...
    private void abrirUsuario(String username) throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        if(username.equals(StEntities.USER.getUsername())){
            OptionPane.showOptionPanel(new OpanelUserData(), OptionPane.titleUserData());
            return;
        }
        if(username.equals("root") && !StEntities.USER.getUsername().equals("root")){
        ...
        staticUser = (User)load.get(username, 0, new User());
            if(staticUser!=null){
                if(staticUser.getTipo() == 1 && StEntities.USER.getTipo() != 1 && StEntities.USER.getTipo() != 7){
    ...
    private void limpiarTextField() {
        ...
        String newPass = HelpUtils.getResetPass();
    ...
    public void resetPass(){
        String pass = HelpUtils.getResetPass();
        try{
            cWT();
            int fila = tblListar.getSelectedRow();
            String username = tblListar.getValueAt(fila, 0).toString();
            if((username.equals("admin") || username.equals("root")) && !StEntities.USER.getUsername().equals("admin")){
