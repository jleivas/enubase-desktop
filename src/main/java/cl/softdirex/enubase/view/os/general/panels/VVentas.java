...
import cl.softdirex.enubase.view.notifications.panels.input.OpanelSelectClient;
import cl.softdirex.enubase.view.notifications.panels.input.OpanelSelectDate;
import cl.softdirex.enubase.view.notifications.panels.input.OpanelSelectUser;
import cl.softdirex.enubase.view.notifications.panels.input.OpanelSelectUserAndDate;
...
public class VVentas extends javax.swing.JPanel {
     ...
     */
    public VVentas(int filter) throws SQLException, ClassNotFoundException {
        if(!ContentAdmin.lblTitle.getText().toLowerCase().contains("venta")){
            ContentAdmin.lblTitle.setText("Ventas");
        }
        initComponents();
    ...
    private void initComponents() {
        ...
        btnExportVentas = new javax.swing.JLabel();
        ...
        btnExportVentas.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/icons8_Report_Card_50px.png"))); // NOI18N
        btnExportVentas.setToolTipText("Exportar fichas");
        btnExportVentas.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                btnExportVentasMouseClicked(evt);
            }
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                btnExportVentasMouseEntered(evt);
            }
            public void mouseExited(java.awt.event.MouseEvent evt) {
                btnExportVentasMouseExited(evt);
            }
        });
    ...
    private void btnAbrirMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAbrirMouseClicked
        try{
            ...
            Venta venta = (Venta)GV.searchInList(codigo, BDUtils.getVentas(), new Venta());
            StEntities.setOpenVenta(venta);
            boton.venta();
            cDF();
        }catch(Exception e){
        ...
        if(GV.tipoUserAdmin()){
            try{
                ...
                if(OptionPane.getConfirmation("Eliminar registro", "¿Esta seguro que desea eliminar el folio "+folio+"?", 2)){
                    cWT();
                    if(load.delete(folio,0, new Venta()))
                        OptionPane.showMsg("Anular Venta", "La venta ha sido anulada", 1);
                    else
                        OptionPane.showMsg("Anular Venta", "No se pudo anular la venta", 2);
                    cDF();
                }
                load();
                ...
                OptionPane.showMsg("Seleccione Venta","Error al cargar valores,\n"
    ...
    private void btnExportVentasMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnExportVentasMouseClicked
        if(GV.licenciaExpirada()){
            GV.mensajeLicenceExpired();
        }else{
            if(BDUtils.getVentas().size()>0){
                cWT();
                GV.printVentas(BDUtils.getVentas());
                cDF();
            }else{
                mensajeOperacionCanceladaPorTablaVacia();
            } 
        }
    }//GEN-LAST:event_btnExportVentasMouseClicked

    private void btnExportVentasMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnExportVentasMouseEntered
        btnExportVentas.setIcon(new javax.swing.ImageIcon(getClass().getResource(Icons.getEnteredIcon(btnExportVentas.getIcon().toString()))));
    }//GEN-LAST:event_btnExportVentasMouseEntered

    private void btnExportVentasMouseExited(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnExportVentasMouseExited
        btnExportVentas.setIcon(new javax.swing.ImageIcon(getClass().getResource(Icons.getExitedIcon(btnExportVentas.getIcon().toString()))));
    }//GEN-LAST:event_btnExportVentasMouseExited
...
//                try {
//                    cWT();
//                    GV.ventasToDelivery(BDUtils.getVentas());
//                    cDF();
...
//        }
    ...
    private void btnExportExcelMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnExportExcelMouseClicked
        if(GV.tipoUserAdmin()){
            ...
            XlsUtils.exportarVentasAExcel(BDUtils.getVentas());
    ...
    private void btnRestaurarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnRestaurarMouseClicked
        OptionPane.showMsg("Funcion no disponible", "Esta opción no se encuentra disponible\n"
                + "debe generar una venta nuevamente", 3);
//        if(GV.tipoUserAdmin()){
//            try{
//                cWT();
//                int fila = tblListar.getSelectedRow();
...
//                    cWT();
//                    if(load.restore(folio,0, new Venta()))
//                    OptionPane.showMsg("Restaurar Venta", "La venta ha sido restaurada", 1);
//                    else
//                    OptionPane.showMsg("Restaurar Venta", "No se pudo restaurar la venta", 2);
//                    cDF();
//                }
//                load();
//
...
//        }
    ...
    private javax.swing.JLabel btnExportVentas;
    ...
    private void load(){
        ...
        GlobalValuesVariables.setCboVentasFilter(cboFilterOptions.getSelectedIndex());
        cargarDatos(index);
    }
    ...
    private void cargarDatos(int status) {
        ...
        if(filter==BY_DAY){
            ...
            BDUtils.listarVentasByDate(new Date(),null);
        }
        if(filter==BY_DATE){
            if(isFiltering){
                ...
                BDUtils.listarVentasByDate(GlobalValuesVariables.getDateFrom(),GlobalValuesVariables.getDateTo());
            }
            String fecha1 = GV.dateToString(GlobalValuesVariables.getDateFrom(), "dd/mm/yyyy");
            String fecha2 = GV.dateToString(GlobalValuesVariables.getDateTo(), "dd/mm/yyyy");
        ...
        if(filter==BY_CLIENT){
            if(isFiltering){
                ...
                BDUtils.listarVentasByClient(GlobalValuesVariables.getRutClienteSelected());
                try {
                    Cliente cli = (Cliente)load.get(GlobalValuesVariables.getRutClienteSelected(), 0, new Cliente());
        ...
        if(filter==BY_USER){
            if(isFiltering){
                ...
                BDUtils.listarVentasByUser(GlobalValuesVariables.getIdUserSelected());
                try {
                    User us = (User)load.get(null, GV.strToNumber(GlobalValuesVariables.getIdUserSelected()), new User());
        ...
        if(filter==BY_USER_DATE){
            if(isFiltering){
                ...
                BDUtils.listarVentasByUserAndDate(GlobalValuesVariables.getIdUserSelected(),GlobalValuesVariables.getDateFrom(),GlobalValuesVariables.getDateTo());
                try {
                    User us = (User)load.get(null, GV.strToNumber(GlobalValuesVariables.getIdUserSelected()), new User());
...
//                if(BDUtils.getVentas().size()>0){
//                    cWT();
//                    GV.printSalesReport(BDUtils.getVentas(), ContentAdmin.lblTitle.getText());
//                    cDF();
//                }else{
//                    mensajeOperacionCanceladaPorTablaVacia();
//                }
...
//                int filter = cboFilterOptions.getSelectedIndex();
//                if(filter == BY_USER_DATE){
//                    if(GlobalValuesVariables.getIdUserSelected().equals(""+StEntities.USER.getId())){
//                        cWT();
//                        GV.printSalesReport(BDUtils.getVentas(), ContentAdmin.lblTitle.getText());
//                        cDF();
//                        return;
