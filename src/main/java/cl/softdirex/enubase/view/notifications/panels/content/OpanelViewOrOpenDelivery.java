...
public class OpanelViewOrOpenDelivery extends javax.swing.JPanel {
    ...
    private void btnGuardarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnGuardarMouseClicked
        CursorUtils.cursorWAIT(this);
            ...
            if(validaData()){
                try {
                    Despacho dsp = new Despacho(null, rut.replace(".", ""), GV.getToName(name), fecha, StEntities.getOpenVenta().getCod(), 1, null, 0);
                    load.add(dsp);
                    StEntities.getOpenVenta().setDespacho(dsp);
                    StEntities.getOpenVenta().setEstado(GlobalValuesVariables.estadoVentaDelivered());
                    load.update(StEntities.getOpenVenta());
    ...
    private boolean validaFecha() {
        ...
        if(GV.dateToString(fecha, "ddmmyyyy")
                .equals(GV.dateToString(StEntities.getOpenVenta().getFecha(), "ddmmyyyy"))){
            return true;
        }
        if(!fecha.after(StEntities.getOpenVenta().getFecha())){
    ...
    private void loadData() {
        Despacho ds = StEntities.getOpenVenta().getDespacho();
