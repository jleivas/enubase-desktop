    ...
    */
    public boolean add(Object object) throws InstantiationException, IllegalAccessException {
        ...
        if(NetWrk.isOnline()){
            ...
            if(object instanceof SyncStringId){
                ...
                Notification.showMsg("No se puede crear nuevo registro", "Para poder ingresar un nuevo registro debes tener acceso a internet."+object.toString(), 2);
            }
        }
        return false;
    }
    ...
    public boolean updateFromUI(Object object){
        if(validaEntity(object)){
            if(GV.licenciaLocal()){
                Notification.showMsg("NO CREATED", "function ist created!", 3);
                return false;
            }else{
                updateRemote(object);
                return true;
        ...
        try{
            ...
            if(temp != null){
                ...
                if(newStock >= 0){
                    ...
                    Notification.showMsg("No se pudo reducir el stock", "La cantidad a reducir es mayor que el stock disponible", 2);
                }
            }
            return false;
    ...
    public boolean restoreOrDeleteFromUI(Object object){
        if(!validaPrivilegiosParaEstados(object)){
            if(((SyncClass)object).getEstado() == 0){
                Notification.showMsg("No tienes privilegios suficientes", "No tienes permiso para restaurar el registro", 2);
                return false;
            }else{
                Notification.showMsg("No tienes privilegios suficientes", "No tienes permiso para eliminar el registro", 2);
                return false;
            }
        }
            ...
            if(type instanceof SyncClass){
                if(type instanceof Venta){
                    if(GV.fechaPasada(((Venta)temp).getFecha())){
                        Notification.showMsg("No se puede eliminar", "Esta opción no se encuentra disponible,\n"
                ...
                Notification.showMsg("No se puede eliminar", "El registro no tiene tipo válido.", 2);
                return false;
            }
        }else{
            Notification.showMsg("No se puede eliminar", "El registro no existe.", 2);
            return false;
        } 
    }
    ...
    public boolean restore(String cod,int id,Object type) throws SQLException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        if(temp != null){
            if(type instanceof SyncClass){
                if(type instanceof Venta){
                    Notification.showMsg("No se puede restaurar", "Esta opción aún no se encuentra disponible.", 2);
                    return false;
                    //((SyncClass)temp).setEstado(((((SyncClass)temp).getEstado())*-1));
                }else{
                ...
                Notification.showMsg("No se puede restaurar", "El registro no tiene tipo válido.", 2);
                return false;
            }
        }else{
            Notification.showMsg("No se puede restaurar", "El registro no existe.", 2);
            return false;
        } 
    }
    ...
    private void msgInvalidName(int status){
        if(status == 0){
            Notification.showMsg("Imposible registrar elemento", "Ya existe un registro con el nombre ingresado pero se encuentra elimidado,\n"
                + "debes cambiar el nombre para poder continuar o restaurar el elemento eliminado.", 2);
        }else{
            Notification.showMsg("Imposible registrar elemento", "Ya existe un registro con el nombre ingresado,\n"
    ...
    private void msgEntityNotAdded(){
        Notification.showMsg("Proceso finalizado", "El registro no se ha podido guardar\n"
